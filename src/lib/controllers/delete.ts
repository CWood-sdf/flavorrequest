...
export async function deleteElement(params: { id: string }, cookies: Cookies, collection: string) {
    const id = parseInt(params.id);
    await assertAdmin(cookies);
    //delete the admin
    const data = await firestore.collection(collection).where("id", "==", id).get();
    //delete the admin
